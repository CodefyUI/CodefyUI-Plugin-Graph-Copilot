    ...
    input_schema: {
      ...
      properties: {
        operations: {
          type: "array",
          items: { type: "object" }
          // GraphOp union documented in description
        }
      },
      required: ["operations"]
    }
...
function Ph({
  ...
  el.useEffect(() => () => {
    ...
    U === "openai-codex" && Cr(d).then((C) => {
      C.status === "authenticated" && M(!0, C.email ?? null);
    }).catch(() => {
    });
  }, []);
  function il(C) {
  ...
  async function w() {
    try {
      ...
      window.open(C, "_blank"), yl(!0), Bl.current = Date.now(), tl.current = setInterval(async () => {
        try {
          const El = await Cr(d);
          El.status === "authenticated" && (M(!0, El.email ?? null), yl(!1), tl.current !== null && (clearInterval(tl.current), tl.current = null));
        } catch {
        }
